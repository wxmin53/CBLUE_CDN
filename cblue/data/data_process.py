...
class CDNDataProcessor(object):
    def __init__(self, root, recall_k=200, negative_sample=3):
        ...
        self.label2id, self.id2label = self._get_labels()  # label为标准数据集，添加了未被包含的训练集和验证集
    ...
    def get_test_sample(self, dtype='cls'):
        ...
        if dtype == 'cls':
            # recall_orig_samples = {'text':[text1, text2, ...],
            #                        'recall_label': [[[],[],...], [[],[],...]],
            #                        'label': [[0],...]
            # outputs = {'text1':[text1, text1, ..., text2, ...],
            #            'text2':[label11, label12, ..., label21, ...],
            #            'label':[0, 0, ..., 0, ...]}
            # recall_orig_samples_scores = [[t0k0, t0k1, t0kn], ..., [tmk0, tmk1, tmkn]]
    ...
    def _get_cls_samples(self, orig_samples, mode='train', do_augment=1):
        ...
        # 计算tfidf + 预测/训练数据texts与xx余弦相似度 --> 返回top k的label的index和score
        # recall_samples_idx = [[t0k0, t0k1, ...t0kn], [t1k0, t1k1, ...t1kn], [tmk0, tmk1, ...tmkn]]
    ...
    def _recall(self, texts):
        """
        recall_scores_idx：分数从大到小排序后，获取分数的原始索引
        recall_scores：分数从大到小排序
        """
